...
{   
  ...
  def eval(pc: Piece, m: Move, en: Int, b: Board) : Set[Piece] = 
  {
    if (!inside(pc.pos, b)) Set()
      ...
      if (!is_occupied(pc.pos, b) || 
          (is_occupied(pc.pos, b) && !occupied_by(pc.pos, b).contains(pc.col))) 
        Set(pc)
      else Set()
    } else if (is_occupied(pc.pos, b)) Set()
    else m match {
  ...
  {
    val modifiedBoard = b - pc 
    val directions = List(U, D, R, L, RU, LU, RD, LD, UR, UL, DR, DL)
    directions.flatMap(move => eval(pc, move, pc.en, modifiedBoard)).toSet
  }
  ...
  {
    val attackMoves = b.pces
      .filter(_.col == c)
      .flatMap(pc => all_moves(pc, b))
      .map(_.pos)
      .toSet
    b.pces.filter(p => p.col != c && attackMoves.contains(p.pos))
  }
  ...
  {
    val enemies = b.pces.filter(_.col != pc.col)
    enemies.toList.flatMap(enemy => all_moves(enemy, b))
          .count(move => move.pos == pc.pos)
  }
  ...
  {
    val allies = b.pces.filter(_.col == pc.col) - pc 
    allies.toList.flatMap(ally => all_moves(ally, b - pc))
          .count(move => move.pos == pc.pos)
  }
  ...
  {
    case Pawn(_, _, _) => all_moves(pc, b)
    case King(_, _, _) =>
      val possibleMoves = all_moves(pc, b)
      possibleMoves.filter { move =>
        val isNotAttacked = attackedN(move, b) == 0
        lazy val targetPiece = occupied(move.pos, b) 
        lazy val isNotProtected = targetPiece.isEmpty || 
                                protectedN(targetPiece.get, b) == 0
        isNotAttacked && isNotProtected
      }
  }
