...
{ 
  ...
  {
    """\w+""".r.findAllIn(s.toLowerCase).toList
  }
  ...
  {
    xs.groupBy(identity).map { case (str, list) => (str, list.length) }
  }
  ...
  def prod(lst1: List[String], lst2: List[String]) : Int = 
  {
    ...
    val allStrings = (occ1.keySet ++ occ2.keySet).toList
    
    allStrings.map { str =>
      occ1.getOrElse(str, 0) * occ2.getOrElse(str, 0)
    }.sum
  }
  ...
  {
    val dotProduct = prod(lst1, lst2)
    val d1_squared = prod(lst1, lst1)  
    val d2_squared = prod(lst2, lst2)  
    
    dotProduct.toDouble / math.max(d1_squared, d2_squared)
  }
