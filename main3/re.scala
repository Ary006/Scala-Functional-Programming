...
object M3 
{
  ...
  def der(c: Char, r: Rexp) : Rexp = r match
  {
    ...
    case SEQs(Nil) => ZERO
    case SEQs(r :: rs) =>
      if (nullable(r)) ALT(SEQs(der(c, r) :: rs), der(c, SEQs(rs)))
      else SEQs(der(c, r) :: rs)
    case STAR(inner) => SEQ(der(c, inner), STAR(inner))
  }
  ...
  def denest(rs: List[Rexp]) : List[Rexp] = rs match 
  {
    ...
    case ALTs(inner) :: rest => inner ::: denest(rest)  
  ...
  def flts(rs: List[Rexp], acc: List[Rexp] = Nil) : List[Rexp] = rs match 
  {
    ...
    case SEQs(inner) :: rest => flts(rest, acc ::: inner)
